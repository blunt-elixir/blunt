defmodule Blunt.Message.Compilation do
  @moduledoc false
  require Logger

  alias Blunt.Message.Compilation
  ...
  def log(module, action, elapsed_milliseconds \\ nil) do
    cond do
      Blunt.Config.log_when_compiling?() == true ->
        if elapsed_milliseconds,
          do: Logger.info("[blunt] #{action} #{inspect(module)} (#{elapsed_milliseconds} ms)"),
          else: Logger.info("[blunt] #{action} #{inspect(module)}")
